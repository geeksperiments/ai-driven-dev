...
- Step 1: In brownfield-backend, create a new Entity class named Owner in owner package. It should have 3 attributes (id, name, address). Define its schema in data.sql. Verify by running `mvnw compile` ✅

- Step 2: Refactor Pet entity to replace ownerName field with @ManyToOne Owner relationship. Update Pet schema and test data in data.sql to use foreign key references. Verify by running `mvnw compile` ✅

- Step 3: Update Visit entity to add @ManyToOne Owner relationship. Modify Visit schema and test data accordingly. Verify by running `mvnw compile` ✅

## Repository Layer Updates

- Step 4: Create OwnerRepository interface extending JpaRepository with findByName method. Update PetRepository to add findByOwner method. Verify by running `mvnw compile` ✅

- Step 5: Update VisitRepository to add findByOwner method. Verify by running `mvnw compile` ✅
